...
> | | |
> |---|---|
> | **Next step** | **11.2 · `--solo`/`--mute`/`--stems` in `tfmx-cli render`**, plus the listening pass on `apidya (title)` and the suspect `turrican` module. |
...
> | **Last done** | 11.1 · Voice mute in `Paula`: `muted: [bool; 4]` field, `set_voice_muted(voice, bool)`, forwarding `Player::set_voice_muted`. Muting happens at the mix inside `render()` — `next_sample` still runs unconditionally so a muted voice's playback position, loop completions and attack→loop handoff evolve identically; only the `+=` into the pan accumulators is skipped. `Paula::voice(&self, voice) -> Voice` promoted from `#[cfg(test)] pub(crate)` to a public getter, ahead of 11.3's need for it. New tests: muting silences a single voice; a muted voice's `frac` still advances; all four solo renders sum sample-for-sample to the full mix on non-clipping (flat-source, low-volume) material. Golden hashes and existing test suite unchanged; `tfmx` still builds for `wasm32-unknown-unknown`. |
>
