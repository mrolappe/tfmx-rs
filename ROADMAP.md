...
> | | |
> |---|---|
> | **Next step** | **The synthetic-module A/B against `uade123` was attempted and shelved** (not abandoned -- two real, previously-undocumented `uade123` invariants were found by bisection and recorded for reuse, but its rendered audio showed zero measurable difference between offset variants for voices 1-3, root cause not found without reading GPL source; this crate's own side reproduced the designed 2-jiffy lag exactly). **Also resolved this session**: a note superseded before reaching its own `$01 DMAon` is not silently dropped -- `MacroInterpreter::trigger()` forces DMA off (and clears vibrato/portamento/envelope) the instant any genuine trigger runs, so it audibly cuts the previous sound short even when its own content never sounds; measured over `turrican intro`, these cuts are short (1-3 jiffies), not a basis for a new missing-audio bug class. Neither of these touched the still-open "different melody" phase-gate complaint. **Recorded, not yet done**: re-run `tfmx-cli render --stems`/`--solo`/`--mute`/`lint` at t=1.28s/t=2.00-2.08s specifically (not redone since the two retrigger fixes landed) -- the most concrete untried lead. Final correctness sign-off still needs the user's ear. Full account in `docs/status.md`'s two "Update (2026-07-26, next session)" sections following the synthetic-module plan. |
