...
> | | |
> |---|---|
> | **Next step** | **11.3 · `TraceEvent` and `Player::render_traced`** — the trace seam threaded through `run_jiffy`/`dispatch_pattern_entry`. |
...
> | **Last done** | 11.2 · `tfmx-cli render` gained `--solo N`, `--mute N[,N..]`, `--stems` (derives `out-v0.wav`…`out-v3.wav` from `-o` and runs four solo renders); `resolve_mute_mask`/`derive_stem_paths` as pure, unit-tested functions, `--solo` wins if both given. Manual check: stems of `apidya (title)` and `turrican intro` sum to the full mix exactly on non-clipping samples, with the only divergences explained (±1 LSB rounding-order noise on non-clipped material; large diffs exactly where the full mix's `i16` clamp kicks in and stems individually don't). **Listened to and recorded in `docs/status.md`**: `apidya (title)`'s v0–v2 all loop the same sample fragment, v3 is silent; `turrican intro`'s four voices sound plausible in isolation (bass, a sparse voice, melody, drums) with nothing obviously broken by ear alone. Investigating further surfaced that `uade123 -g` free-of-cost format detection identifies `apidya (title)` as **TFMX 7V** (unsupported by this crate) while every other corpus file is TFMX/TFMX Pro — recorded in `docs/status.md` as a probable root cause for that file specifically. Also rendered `uade123` full-mix reference WAVs for both modules (no per-voice solo exists in `uade123`, checked its full `-h`), the "actual `uade123` A/B" the open issue had been waiting on — still pending the user's own listen to confirm/rule out a `turrican intro` bug. |
>
