...
> | | |
> |---|---|
> | **Next step** | **Pivoting the "different melody" search from `turrican intro`'s own data to shared code.** The user now reports the same symptom (musical, in-time, but wrong melody/instrument) on other corpus modules too, not just `turrican intro` -- every hypothesis ruled out so far tested that module's *own* composition data, which says nothing about code every module shares. Plan: (A) promote the RMS-envelope onset-diff method (hand-rolled ad hoc three times already) into a committed `tfmx-cli` tool, run it across all 10 non-7V corpus modules against `uade123`, record a divergence table; (B) rank shared-code hypotheses (`note_period()` systematic error; `decode_pattern_entry`'s uncertain voice/volume bit split; pattern/macro-number or track/voice mapping) and post them to the user before testing; (C) instrument only the top surviving hypothesis, TDD, confirmed by ear before calling it done. Full account in `docs/status.md`'s "Plan (2026-07-26, next session): pivoting from `turrican intro`'s own data to shared code" section. |
