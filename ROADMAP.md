...
> | | |
> |---|---|
> | **Next step** | **Test whether the 160ms per-note attack lag is systemic, not just a first-note artifact.** This session fixed a second real retrigger bug (voice 3's keysplit/`$06 Cont` instrument, `MacroInterpreter::instrument` field, commit `c919266`) which closed the t=0.72s silence gap in `turrican intro`'s first 3s (found via a jiffy-by-jiffy `dma_on` trace across all four voices). Two gaps remained (t=1.28s, t=2.00-2.08s); `tfmx-cli disasm` (new, commit `5c0b35f`) confirmed the macros involved (28, 48) don't use `$06 Cont` -- different mechanism. **The user listened to `uade123`'s reference at both timestamps and confirmed it is NOT quiet there** -- this rules out "composer-intentional rest" and confirms both remaining gaps are real bugs. Re-derived the jiffy math: `$00 aa=0` (1-jiffy pause) followed by a suspending `$08`/`$09 AddNote` puts `$01 DMAon` **two full jiffies after every fresh `trigger()`**, not just the song's first note -- the same mechanism already found and left unexplained for the first-note 160ms gap two sessions ago, just never checked for recurrence. Working hypothesis, **not yet tested**: this 2-jiffy dead zone happens on *every* note restart throughout the piece, and stacks into an audible gap whenever several voices restart close together -- meaning the three gaps found so far could be one systemic timing bug, not three. **Next session's concrete task**: pick several isolated single-note attacks later in the piece, compare this crate's `$01 DMAon` jiffy (via `tfmx-cli trace`) against `uade123`'s audible onset for the same note (seek with `uade123 -s 0 -j <seconds> "testdata/mdat.turrican intro"`) -- if the ~2-jiffy lag recurs consistently, `$00`/`$08`'s suspend semantics (or trigger-jiffy counting) is the next real fix; if inconsistent, don't assume the hypothesis, something else is going on. Full account in `docs/status.md`'s "Update (2026-07-26, same session, continued): added `tfmx-cli disasm`; reference confirms the two remaining gaps are real bugs" section. |
