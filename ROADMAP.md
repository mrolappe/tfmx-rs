...
> | | |
> |---|---|
> | **Next step** | **Build synthetic `.mdat`/`.smpl` file pairs to test the multi-voice-overlap hypothesis as a controlled A/B against `uade123` (and possibly the real TFMX editor via `fs-uae`), instead of continuing to infer it from `turrican intro`'s own dense data.** Last session confirmed the 2-jiffy per-note attack lag is systemic with zero exceptions (785-trigger trace analysis) and very likely genuine TFMX/hardware-faithful behavior (`$00`'s handler is documented `<DMAoff+Reset>`), which reframes the still-open t=1.28s/t=2.00-2.08s silence gaps: they most likely come from *multiple* voices' 2-jiffy dead zones landing on the same jiffy, not from any one voice's lag being wrong. **Plan**: hand-build a minimal module (same raw-byte-poking technique the crate's own tests already use, e.g. `tfmx/src/player.rs`'s `trackstep_master_vol_slide_moves_on_the_first_jiffy`) with 2-4 voices running a macro shaped like the confirmed-linear macro 10 (`$00` pause → setup → `$08 AddNote` → `$01 DMAon`), triggered at controlled jiffy offsets (same jiffy, 1 apart, 2 apart, 3+ apart) with a real audible sample (not silence). Render with this crate and separately capture `uade123`'s output to WAV (`-f out.wav -e wav -j <seconds> -t <secs>`) for objective RMS/onset comparison. This directly tests whether `uade123` also shows the 2-jiffy dead zone on every attack, and whether stacking several voices' attacks produces an audible full-mix gap there too. A synthetic module may also load in the original TFMX editor via `fs-uae` even though `turrican intro` itself didn't -- a third, more-authoritative reference if it works. Lower-priority alongside this: re-run `tfmx-cli render --stems`/`--solo`/`--mute`/`lint` at the two gap timestamps, not redone since the two retrigger fixes landed. Final correctness sign-off still needs the user's ear. Full account in `docs/status.md`'s "Next session's concrete plan (recorded 2026-07-26, not yet started): synthetic modules as a controlled A/B" section. |
