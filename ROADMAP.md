...
> | | |
> |---|---|
> | **Next step** | **Get the user's ear confirmation on this session's fix before doing anything else.** Chased the recurring silence-gap symptom (found via a 10s spectrogram two sessions ago) with a jiffy-by-jiffy `dma_on` trace across all four voices and confirmed it concretely: at t=0.72s, t=1.28s, and t=2.00-2.08s in `turrican intro`'s first 3s, **all four voices are simultaneously silent**. Traced the t=0.72s gap to a second real retrigger bug, same family as `5da3623` but reached through `$06 <Cont>` indirection: voice 3's percussion instrument (macro 24) is a keysplit (`$1C`) that falls through to `$06 <Cont>` into macro 22/23 *within the same jiffy as the trigger* (neither opcode suspends), so by the next retrigger `self.macro_number` has already moved on and the `note_on` fix's "is this instrument still running" check (`macro_number == self.macro_number`) always compares against the wrong thing, forcing a full reset on every fast repeat and never letting `$01 DMAon` run. **Fixed** (TDD, new field `MacroInterpreter::instrument` tracks the pattern-visible triggered macro number separately from the mutable program-counter `macro_number`; `note_on` now compares against `instrument`). Golden-hash diff changed for exactly the three corpus modules sharing this idiom (`turrican intro`, `turrican 2 title (st)`, `turrican 3 level 1`); every other module byte-identical. **Confirmed by re-trace: the t=0.72s gap is gone; t=1.28s and t=2.00-2.08s remain**, traced to a *different*, so-far-unconfirmed-as-bug mechanism (multiple voices' independent, spec-faithful `$00`-pause attack latencies happening to overlap at one trackstep line). Full account in `docs/status.md`'s "Update (2026-07-26, next session): a second retrigger bug found and fixed via next-step (a)" section. **Do not chase the remaining two gaps further until the user has listened and confirmed whether this fix is audible progress.** |
