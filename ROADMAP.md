...
> | | |
> |---|---|
> | **Next step** | None within Phase 11 — **phase gate reached.** All six steps of Phase 11 (M4) are done; stop and wait for explicit approval before choosing the next round (a repair of the `turrican intro` bug, or the export/static-analysis milestone — see "Later milestones"). |
> | **Phase** | 11 of 11 (M4) — Diagnostics tooling — **complete, awaiting phase-gate approval** |
> | **Gate** | M3 complete and approved. **M4/Phase 11 was planned and approved on 2026-07-26** and is now complete; stop here for explicit approval before starting the next round. The open issue in `docs/status.md`'s "Open follow-up" section is what M4 was built to attack. **Reframed by 11.2, now two distinct issues**: (1) `uade123`'s own format detection shows `apidya (title)` is the corpus's only **TFMX 7V** file — a format this crate explicitly does not support (`docs/architecture.md` §9) — a likely-sufficient explanation for its symptom on its own. (2) `turrican intro` (confirmed plain TFMX, this crate's target format) is a **separate, confirmed real bug**: the user's `uade123` A/B by ear found a different melody, a different starting patch, an extra percussion voice, and higher pitch than the reference — pointing at wrong pattern/macro/note dispatch, not mixing. 11.5's `lint` narrowed it further: voice 1 fires 245 note-ons across six macros but only ever DMAs on one of two loaded sample regions (`no-retrigger`). Root cause not yet fixed — M4 delivered *tools and a diagnosis*, not a fix; what to repair is decided here at the gate. |
