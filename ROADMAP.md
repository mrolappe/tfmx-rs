...
> | | |
> |---|---|
> | **Next step** | **The two remaining all-voice silence gaps (t=1.28s, t=2.00-2.16s) are now explained: trackstep-data-driven, not a bug.** `tfmx-cli trace` shows both lines (91 and 100) carry a `StopVoice{voice:1}` plus fresh `Note` triggers for voices 3, 0, and 2 all in the same jiffy -- three simultaneous genuine triggers plus a stop force every voice's DMA off at once for 1-2 jiffies, by construction of the composition data, not from this crate's dispatch order or timing. Any spec-faithful player, including `uade123`, would show the same dip here. This rules both timestamps out as the "different melody" cause. `lint` at 3s: no findings. **No concrete next lead identified yet** -- the remaining discrepancy is most likely in note/pitch/instrument dispatch, not timing/silence. Final correctness sign-off still needs the user's ear. Full account in `docs/status.md`'s "Update (2026-07-26, next session): the two remaining all-voice silence gaps are trackstep-data-driven, not a bug" section. |
