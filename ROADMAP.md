...
> | | |
> |---|---|
> | **Next step** | **The 2-jiffy per-note attack lag is confirmed systemic, with zero exceptions** (785-trigger trace analysis of `turrican intro`'s first 30s, throwaway script, not committed): every genuine, uninterrupted `trigger()` reaches its own `$01 DMAon` exactly 2 jiffies later, no scatter. `$00`'s handler is documented as `<DMAoff+Reset>` -- turning DMA off is its designed job, not a bug -- so this lag is very likely genuine TFMX/hardware-faithful behavior, present in `uade123` too, not an implementation defect. Side-finding: 71% of this piece's note-instrument changes on a given voice are themselves superseded by a further retrigger before their own DMAon ever fires (very fast multiplexing of several logical parts onto 4 Paula channels) -- worth checking separately whether those superseded notes are ever meant to be audible. **Reframed question for the still-open t=1.28s/t=2.00-2.08s gaps**: since each voice's own attack lag now looks uniform and correct, the gaps most likely come from *multiple* voices' 2-jiffy windows landing on the same jiffy -- next session should check whether the trackstep data's own independent per-track timing already puts them there by construction (in which case any faithful player should show the same gap, in tension with the earlier by-ear "not quiet" confirmation, which was at normal listening level and might not register a few overlapping 2-jiffy dips) or whether this crate's dispatch order is nudging otherwise-staggered attacks together. Full account in `docs/status.md`'s "Update (2026-07-26, next session): the 2-jiffy attack lag is confirmed systemic -- with zero exceptions" section. |
