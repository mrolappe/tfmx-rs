...
> | | |
> |---|---|
> | **Next step** | **`turrican intro` still sounds very different from `uade123` after the `note_on` fix below — that fix was real and is committed, but it is not sufficient, or not the (only) actual cause.** Keep digging: candidates not yet checked are transpose handling, note→period mapping, and which pattern/macro is actually selected for the reference's "starting patch" vs. this crate's. Song-number mismatch has been **ruled out** (see Gate row) — the discrepancy is genuinely in note/pitch/patch dispatch. Use `tfmx-cli trace`/`render --stems` to compare voice-by-voice against a fresh `uade123` render, the same way the `note_on` bug was found. |
