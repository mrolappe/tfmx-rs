...
> | | |
> |---|---|
> | **Next step** | **11.6 · Mutation robustness test** — a seeded LCG flips bytes in corpus buffers; `Module::parse` + ~1 s of `Player::render` must never panic. |
> | **Phase** | 11 of 11 (M4) — Diagnostics tooling — in progress |
> | **Gate** | M3 complete and approved. **M4/Phase 11 was planned and approved on 2026-07-26** and may be started; stop at the end of Phase 11. The open issue in `docs/status.md`'s "Open follow-up" section is what M4 exists to attack. **Reframed by 11.2, now two distinct issues**: (1) `uade123`'s own format detection shows `apidya (title)` is the corpus's only **TFMX 7V** file — a format this crate explicitly does not support (`docs/architecture.md` §9) — a likely-sufficient explanation for its symptom on its own. (2) `turrican intro` (confirmed plain TFMX, this crate's target format) is a **separate, confirmed real bug**: the user's `uade123` A/B by ear found a different melody, a different starting patch, an extra percussion voice, and higher pitch than the reference — pointing at wrong pattern/macro/note dispatch, not mixing. Root cause not yet found. Read `docs/status.md`'s "Update (2026-07-26, step 11.2)" section before touching playback correctness. Note M4 delivers *tools and a diagnosis*, not a fix: what to repair is decided at the Phase 11 gate. |
> | **Last done** | 11.5 · `tfmx-cli lint <mdat> <smpl> [--song N] [--seconds S]`, and `info` made static (`tfmx-cli/src/main.rs`). `info` lost its hidden 30 s render and its unsupported-opcode histogram (and its `--song`/`--seconds` args); it is now header text, layout and the song/tempo table only, its test asserting the histogram is *gone*. `lint(&[TraceEvent], &[(u8, u32)], &[i16], rate) -> Report` is the pure function the step calls for: the trace events, the `(opcode, count)` pairs read off `Player::unsupported_ops()` and the rendered PCM go in, a `Report` (summary + findings) comes out. **Design decision on the stated tension**: the unsupported counters and the PCM cannot be derived from the event stream (they live on the player and in the output buffer), so `run_lint` gathers all three from one chunked `render_traced` pass and passes them in side by side rather than adding a `TraceEvent` variant — the trace seam stays what `docs/architecture.md` §2 says it is, state-machine transitions only, and the analysis stays fully testable without a `Player`. Summary: jiffies, tempos, distinct trackstep lines, looped/stopped, distinct patterns per track, distinct macros and note-ons per voice, pattern-command histogram (keyed off the `Debug` variant name, so a new `PatternCommand` needs no change here), unsupported-opcode histogram, PCM peak and clipped-sample count. All eight findings implemented and unit-tested from hand-built event vectors with no corpus and no player (`dead-voice`, `frozen-voice`, `no-retrigger`, `single-pattern`, `stopped-early`, `silence`, `clipping`, `unsupported-ops`), plus negative tests that a healthy voice/render fires nothing. **The deliverable — the corpus-wide table (song 0, 30 s, all 10 modules) — is recorded in `docs/status.md`'s "Update (2026-07-26, step 11.5)" section**, with the reading of it. Headlines: `turrican intro`'s confirmed bug is *not* a stalled sequencer (55 lines, 67 pattern visits, all four voices triggering) but shows `no-retrigger` on voice 1 — 245 note-ons across six macros yet one audible sample region all run, a second region loaded but never DMA'd on: the sharpest lead the corpus offers. `x-out (title)` stops after 6.1 s of 30 with a tempo set of `{7, 65280}` (`$FF00` is not a tempo) — second lead. `turrican 2 title (st)` is clean and is the best known-good baseline. `apidya (title)` flags only its silent voice 3 (`start=0 len=0`); its 7V garbage shows up as degenerate two-word regions rather than as `no-retrigger`, so the finding as specified does not actually catch its flagship symptom. `no-retrigger` is noisy as worded (8 of 10 modules) and must be read with its region detail. `unsupported-ops`, `silence` and `single-pattern` fired nowhere. No change to the `tfmx` core crate. |
>
