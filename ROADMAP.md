...
> | | |
> |---|---|
> | **Next step** | **11.5 · `tfmx-cli lint`** — fold a traced render plus PCM statistics into a corpus-wide diagnostic report; `info` made static. |
...
> | **Last done** | 11.4 · `tfmx-cli trace <mdat> <smpl> [--song N] [--seconds S] [--voice N] [--track N] [--format text]` (`tfmx-cli/src/main.rs`). `write_text_event` formats one `TraceEvent` as one aligned line (`JIFFY`/`TRACKSTEP`/`PATTERN`/`TRIGGER`/`VOICE`, Debug-formatted payloads); `write_trace` is the pure fold over `&[TraceEvent]` that applies `--track` (to `Pattern`) and `--voice` (to `Trigger`/`Voice`) filters and drops a `Voice` event whose state is unchanged from the last one emitted for that voice, tracked per-voice so filtering doesn't disturb dedup. `TraceFormat` is a one-variant `clap::ValueEnum` (`Text`) matched in `run_trace`, the extension point the phase's design decision calls for — JSON/TOON later is a new format function plus one arm, not a trait. `run_trace` collects a chunked `render_traced` run into a `Vec<TraceEvent>` then calls `write_trace`, following `run_info`'s existing injected-`&mut impl Write` shape. New tests: `write_trace` against hand-built `Vec<TraceEvent>` (no corpus, no player) for each event's text shape, `--track`/`--voice` filtering, and Voice dedup (unchanged suppressed, changed re-emitted, tracked independently per voice); a corpus test asserts song 0 vs. song 1 of `turrican intro` produce different trace text. Manually verified against the corpus: output is aligned and greppable, `--voice` narrows to one voice's `Trigger`/`Voice` lines. |
>
