...
> | | |
> |---|---|
> | **Next step** | **Compare the very first `TRIGGER`/`PATTERN` trace events against the reference's audible attack timbre at t=0** — the fallback the previous session's plan named if master volume turned out insufficient. It did: master-volume slide (`$EFFE 0003`/`0004`, pattern `$FA <Fade>`) is now implemented (TDD, reusing `macro_interp::Envelope`'s existing slide mechanic), but the "default to 0 at song start" theory was tried and **falsified** by the corpus itself before any listening was needed — `apidya (level 1)` (confirmed TFMX Pro, not the unrelated 7V `apidya (title)`) never issues a master-volume command and rendered completely silent under that default despite ~280 real note-ons. Corrected default is 64 (no attenuation, matching every module's prior behavior); under that default, `turrican intro`'s own slide (`MasterVolSlideB { divisor: 0, target: 64 }`, trackstep line 75) is a genuine no-op — current already equals target. Golden-hash diff across all 10 corpus files confirms only `apidya (title)` (the already out-of-scope 7V-as-Pro-2.0 garbage case) changed; the other 9, including `turrican intro`, are byte-identical to before this session. Master volume is a real, correctly-implemented mechanism now, but it does not explain `turrican intro`'s drum-at-the-start symptom — full account in `docs/status.md`'s "Update (2026-07-26, next session): master volume implemented, default-0 theory falsified" section. **Do not start the t=0 attack-timbre comparison until the user is ready to resume.** |
