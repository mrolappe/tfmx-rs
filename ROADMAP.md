...
> | | |
> |---|---|
> | **Next step** | **11.4 · `tfmx-cli trace`** — a text CLI over `TraceEvent`, suppressing unchanged `Voice` events. |
...
> | **Last done** | 11.3 · `TraceEvent` (`tfmx/src/trace.rs`, re-exported from `lib.rs`): `Jiffy`/`Trackstep`/`Pattern`/`Trigger`/`Voice` variants, `Voice`'s `state` field reusing `Paula`'s own `Voice` register-state type (the getter step 11.1 promoted for exactly this). `Player::render_traced` alongside a new private `render_inner`; `render()` is `render_inner(out, |_| {})`. `trace: &mut impl FnMut(TraceEvent)` threaded through `run_jiffy` and `dispatch_pattern_entry`; `Trigger` fires from the dispatch site so `voice_of()`'s nibble masking is visible in the trace, not re-derived. `PatternRunner::advance`'s callback now also reports the `(pattern, step)` an entry was fetched from — needed for an accurate per-entry `Pattern` trace event across a jiffy that runs a `$F1`/`$F2`/`$F8` chain, and replaces a workaround one existing test had for the same gap. Added `Player::frames_rendered` so `Jiffy`'s `frame` stays a continuous session-wide timeline across chunked calls, matching how `tfmx-cli` already renders. New tests: `render()`/`render_traced(.., \|_\| {})` bit-identical; a 1s traced render at a ~48 Hz tempo emits 40–60 `Jiffy` events with non-decreasing `frame`; `frame` keeps advancing across separate chunked `render_traced` calls. Golden hashes unchanged — the load-bearing proof the seam is inert. Trace seam recorded in `docs/architecture.md` §2 alongside the register seam. |
>
