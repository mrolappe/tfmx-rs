...
> | **Gate** | M3 complete and approved. **M4/Phase 11 was planned and approved on 2026-07-26** and may be started; stop at the end of Phase 11. The open issue in `docs/status.md`'s "Open follow-up" section is what M4 exists to attack. **Reframed by 11.2**: `uade123`'s own format detection shows `apidya (title)` is the corpus's only **TFMX 7V** file — a format this crate explicitly does not support (`docs/architecture.md` §9) — which is a likely-sufficient explanation for its symptom on its own, separate from any genuine Pro-2.0 playback bug. `turrican intro`'s status is still open and unconfirmed against a real `uade123` reference render. Read `docs/status.md`'s "Update (2026-07-26, step 11.2)" section before touching playback correctness. Note M4 delivers *tools and a diagnosis*, not a fix: what to repair is decided at the Phase 11 gate. |
