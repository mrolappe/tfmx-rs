...
| M5-04 | Zone resolution (`$1C`/`$1D`) | Per-macro `(note range, velocity range) → sample region` table — the spine feeding MIDI, sampler, sample and tracker export | `done`, phase 5.3 | `resolve_zones` in `tfmx-analysis/src/zones.rs`; `turrican intro` macro 28 and a synthetic probe both check out. Its `$1D` polarity finding is resolved (2026-08-02): confirmed backwards against the real TFMX editor and fixed in `macro_interp.rs`/`zones.rs` — macro 5's chain now resolves to a clean 5-way velocity fan-out. See `docs/m5-session-log.md`'s Phase 5.3 entry. |
| M5-05 | JSON dump + serialization seam | `tfmx-cli dump --format json` and `trace --format json`, filling the existing TODO at `tfmx-cli/src/main.rs:766` | `done`, phase 5.4 | serde stays optional and encapsulated per the serialization decision above; `tfmx-analysis`'s types (`WalkResult`, `Zone`/`ZoneTable`) get feature-gated `Serialize` derives, `tfmx`'s dependency-free core does not — `TraceEvent` is hand-encoded in `tfmx-cli/src/serialize.rs` instead. See `docs/m5-session-log.md`'s Phase 5.4 entry. |
| M5-06 | MIDI export | `tfmx-cli export-midi` with an auto-drafted, hand-editable mapping file; independent ear-oracle for the open `MIDDLE_C_NOTE` pitch question | `done`, phase 5.5 | Does not depend on sample fidelity, unlike every prior verification path in this project. `midi_mapping.rs`/`midi.rs` in `tfmx-cli`; 1 jiffy = 1 MIDI tick, tempo meta events on tempo change, vibrato/portamento surfaced as pitch bend from observed `Voice.period` deltas. See `docs/m5-session-log.md`'s Phase 5.5 entry. |
| M5-07 | Fidelity scoreboard | Batch-rendered distance metrics against reference material, tracked as a committed metrics file | `done`, phase 5.6 | `tfmx-cli fidelity-scoreboard` batch-renders the corpus and `uade123` side by side and scores each with the existing `onset-diff`/`measure-pitch` metrics; JSON committed at `docs/fidelity-scoreboard.json`. Explicitly regression detection, not a truth oracle — this project's history shows structural metrics moving while the ear did not, recorded in the scoreboard's own `honesty_note` field. Its `our_pitch_hz`/`reference_pitch_hz` fields turned out to reproduce the same full-mix autocorrelation-collapse failure this project already found for `measure-pitch` (constant, meaningless ~8820Hz on 9 of 10 modules) — documented in the honesty note rather than silently reported. See `docs/m5-session-log.md`'s Phase 5.6 entry. |
| M5-08 | Sample + sampler-instrument export | WAV with `smpl` loop chunk, SFZ, DecentSampler `.dspreset`, behind one `InstrumentSerializer` trait | `done`, phase 5.7 | `tfmx-cli export-instruments` in a new `tfmx-cli/src/export/` module: `build_instrument` resolves 5.3's zone table to per-zone PCM via `Module::sample`, `InstrumentSerializer` dispatches to `wav`/`sfz`/`dspreset` by name (`export::by_name`, `export::FORMAT_NAMES`). Extended `tfmx_analysis::SampleRegion` with a `looped` field (threaded from `SamplePointer::loop_active`) so exporters know one-shot vs. sustain-loop per zone. WAV loop points hand-rolled (`hound` has no custom-chunk support); verified against macOS `afinfo` and `xmllint`, not just this crate's own round-trip tests. See `docs/m5-session-log.md`'s Phase 5.7 entry. |
| M5-09 | Visualization | View-model structs (waveform regions, loop points, pattern→macro graph, trackstep structure map) with a thin HTML renderer as one consumer | `done`, phase 5.8 | Data collection is the deliverable; the HTML is replaceable |
