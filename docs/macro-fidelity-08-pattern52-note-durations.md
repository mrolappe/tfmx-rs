...
**Status: root cause found, ear-confirmed, fix NOT chosen. One approach tried and ruled out by ear (2026-08-04, session below) — reverted, not committed.** Distinct from [the macro 28 pitch saga](macro-fidelity-05-macro28-pitch-saga.md) (that thread is closed) — this is a separate bug in the same pattern/macro, found afterward.

**▶ START HERE (2026-08-04, later session): next session's task is the shadow-peek approach, not tick-ahead.** Read "Update (2026-08-04): tick-ahead tried, ear-confirmed WORSE, reverted" below in full before writing any code — it explains exactly why the naive fix makes this worse, not better, and the "For whoever picks this up next" section's item 3 (regression tests for all four pinned cases) still applies.
...
## Update (2026-08-04, later session): "tick-ahead, then decide" tried, structurally correct, ear-confirmed WORSE — reverted, not committed

Read `docs/status.md`'s original rationale for the three already-fixed cases first, per the item above (macro 41: pre-attack swallow, needed because a 1-jiffy retrigger cadence can never survive `$00 aa=0`'s pause otherwise; macro 38: no `$14`, so once `dma_on` is true a retrigger is always a genuine new percussive hit; macro 8: `$14`-parked but envelope-decaying, so `$14` alone isn't sufficient evidence of a real sustain). Confirmed via a throwaway instrumented test (reverted, `git status` clean) that macro 41's *real* corpus bytecode — unlike the simpler synthetic shape its own regression tests use — already restarts periodically today at its own 1-jiffy cadence (`dma_on` sequence `false,false,true,false,false,true,...`), so the boundary-tie behavior this thread is chasing is not a protected invariant for macro 41 either.

**Approach implemented (then reverted): `note_on` ticks the still-running instrument itself, synchronously, before deciding sustain vs. restart**, instead of reading `self.dma_on`/`self.wait` as of the end of the *previous* jiffy (stale by one jiffy, since `run_jiffy` dispatches pattern notes before that jiffy's own macro tick — a global reorder was ruled out up front, since it would shift every note's attack onset by a jiffy corpus-wide). `note_on`'s signature grew `module`/`paula`/`voice`/`unsupported`/`emit` (mirroring `MacroInterpreter::tick`) so it could tick itself; `run_jiffy` (and `tfmx-cli`'s standalone `dispatch_pattern_entry_standalone`) gained a per-jiffy `dispatched: [bool; 4]` so the later unconditional per-voice tick loop skips any voice `note_on` already ticked. New unit test `note_on_pattern_82_gives_five_distinct_attacks_at_its_real_cadence` (macro 28's and pattern 82's real bytecode/cadence) went red-then-green; full workspace suite, clippy, `wasm32-unknown-unknown` build and `mutation_robustness` all passed; golden hashes changed for 5 of 10 corpus modules (expected — the heuristic change is general, not pattern-82-specific).

**But the rendered audio is worse, not better — confirmed both quantitatively and by ear.** `onset-diff` on an isolated 6s render of pattern 82 alone: 16 onsets before the change, only **10** after (correlation -0.158, not just weaker but a different rhythm). The user listened to both renders directly and confirmed: **the lower notes are missing/merged in the after-fix version** — the opposite of the intended fix.

**Root cause of the regression, found via temporary `[DEBUG-b7e1]` instrumentation (reverted) printing the sustain decision each dispatch**: every dispatch in pattern 82 does correctly decide "restart" now (matching editor ground truth) — the *decision logic* is right. The bug is that `MacroInterpreter::tick` has a real, committing side effect on `Paula` (`paula.set_dma`/`set_period`/etc. at its end), not just on `self`. For a retrigger landing at or near macro 28's own 2-jiffy attack latency, the sequence within *one* `note_on` call is: peek-tick the old program → it reaches `$01 DMAon` and writes `paula.set_dma(voice, true)` as a real side effect → the sustain check reads `dma_on=true` → decides "not sustaining" (restart) → `trigger()` resets `self` → a second, fresh tick runs `$00 aa=0` → writes `paula.set_dma(voice, false)` again, **in the same jiffy**, overwriting the first write before `Paula::render` ever consumes it for that jiffy's audio. The momentary `true` never reaches the actual output — a genuine restart's audible attack transient is self-cancelling whenever the retrigger gap is close to the macro's own attack latency, which is exactly pattern 82's case (gaps of 2, 2, 4, 2, 4 against a 2-jiffy latency). This is a structural problem with *any* "peek via a real, committing tick" design, not a bug specific to this implementation's bookkeeping.

**Reverted in full** (`git checkout` on `tfmx/src/macro_interp.rs`, `tfmx/src/player.rs`, `tfmx-cli/src/main.rs`, `tfmx-cli/tests/golden.txt`) — full workspace suite confirmed green again on the reverted tree before stopping. Nothing from this attempt is committed.

**Chosen next step, decided with the user**: the shadow-peek approach (discussed and initially set aside for tick-ahead, before this evidence existed) — clone `MacroInterpreter` (needs `#[derive(Clone)]`, not currently on it) and a scratch `Paula` (also needs `Clone`, not currently on it) purely for the decision peek, run the peek tick on the *clone*, decide sustain vs. restart from the clone's resulting state, then discard the clone and let the *real* `self`/`paula` receive exactly one committing tick — either the in-place sustain update (no tick needed, matching today's behavior) or a full `trigger()` + one real tick (matching a genuine restart), never both. This avoids the self-cancelling double-write by construction, since the peek's side effects never touch the real `Paula` the renderer reads. Watch for: cloning `Paula`'s `loop_completions`/other per-voice state accurately enough that the peek's `$1A <Wait on DMA>` (if any macro's early bytecode uses it) doesn't diverge from truth — unlikely to matter for the four pinned cases (none use `$1A` before their own `$01`/`$14`), but not proven either. Must still, per the item above: pin all four cases (macro 41, 38, 8, pattern 82) in one regression pass, and re-run the differential-render + ear-confirmation cycle before calling anything done.

## Also recorded this session, unrelated to this bug: write a comprehensive replayer/playback-loop document

The user asked for a from-scratch, easily-readable document (diagrams/visualizations welcome, no loss of important detail) explaining the *whole* replayer/replay loop end to end: how `Player`, the trackstep sequencer, pattern runners and the macro interpreter fit together, what happens in what order each jiffy, and how the opcodes drive it. This is a documentation task independent of the pattern-82 bug — existing docs (`docs/playback-model.md`, `docs/architecture.md`, `docs/opcodes.md`) hold the pieces but not as one connected, example-driven walkthrough. Not started. Good source material: `Player::run_jiffy` (`tfmx/src/player.rs`) is the actual single-jiffy order of operations to document faithfully (trackstep → per-track pattern dispatch → macro tick → mixer), and this session's own investigation is a fresh, concrete illustration of exactly the kind of ordering subtlety (dispatch-before-tick) such a document should make legible at a glance.
